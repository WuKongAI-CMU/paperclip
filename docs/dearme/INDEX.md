...
Last updated: 2026-05-21 (post DM-GOAL-AUDIT-PUBLIC-FIRST-RUN-MARKERS)
...
- **DM-ABOUT-FOUNDER-PROOF-RAIL** — the founder-story page now turns trust into action with a customer-zero proof rail linking to the dogfood feed and $29 beta pricing, tracked with safe source-only analytics.
- **DM-PRICING-FEED-PROOF-RAIL** — the pricing page now adds a body-level customer-zero proof rail to the dogfood feed and proof archive, with source-only analytics that keep URLs, email, and customer identifiers out of the event payload.
...
- **DM-GOAL-AUDIT-HOSTED-CHECKOUT-REQUIRED** — `pnpm dearme:goal-audit` now treats the first-$29 hosted checkout path as a required completion item, so missing payment-link or signed receipt-sync facts keep the commercial checklist and overall goal blocked until Peter-owned checkout setup is present.
...
- **DM-PROOF-FALLBACK-PRICING-CTA** — proof feed empty/error states now offer a direct beta-pricing handoff and record safe fallback-click analytics without feed contents or customer identifiers.
...
- **DM-PRICING-VIEW-ANALYTICS** — pricing now emits safe `pricing_viewed` funnel analytics on page render, closing the declared activation-to-payment funnel step without logging customer identifiers or starting payment.
- **DM-MANUAL-FIRST-PAYMENT-29** — onboarding manual paid-beta receipts now default to the $29 beta offer and active paid-beta UI fixtures/receipts use $29, keeping the first-payment path aligned with pricing without creating live checkout sessions or logging customer identifiers.
- **DM-PAYMENT-ENV-TEMPLATE-29-OFFER** — the generated hosted-checkout env template now names the live $29/month DearMe offer requirement and reminds operators to keep payment secrets local/server-side before self-serve checkout claims.
- **DM-DAILY-SUMMARY-DEPENDENCY-HELP-QUEUE** — the daily Plain summary now points dependency-only review queues at `docs/NEEDS_HUMAN_HELP.md`, so Peter still has the review templates when no owner-proof, checkout, or Plain-delivery facts are missing.
...
- **DM-DAILY-SUMMARY-BLOCKER-FORMAT** — `pnpm dearme:daily-plain-summary` now formats goal-completion blocker details as blocked-by facts with normalized punctuation, keeping Peter's daily blocker list easier to scan without hiding the exact owner-held facts.
- **DM-PAYMENT-READINESS-JSON-BLOCKER-LABELS** — `pnpm dearme:payment-readiness -- --json` now labels hosted-checkout secret blockers as sensitive hidden values, matching operator text without exposing payment secrets or changing the raw readiness model.
- **DM-DAILY-PLAIN-SUPPORT-SAFE-BLOCKERS** — `pnpm dearme:daily-plain-summary` now renders live-proof blockers with support-safe labels, omits repository URLs from shipped entries, and points shared-message proof commands back to the human-help queue instead of printing raw internal targets.
- **DM-STATUS-SAFE-BLOCKER-LABELS** — `pnpm dearme:status` now renders blocked-target summaries and shared Telegram/iMessage focus text with support-safe labels while preserving exact guarded operator commands.
...
- **DM-RELEASE-GATE-SAFE-OWNER-FACT** — `pnpm dearme:release-gate` now renders the owner-held phone-message proof fact with a support-safe label in human-readable output while preserving exact internal fact names and guarded commands.
- **DM-STATUS-SAFE-OWNER-FACT** — `pnpm dearme:status` now renders the owner-held phone-message proof fact with a support-safe label in human-readable output while preserving exact internal fact names and guarded commands.
- **DM-STANDING-LOOP-SAFE-OWNER-FACT** — `pnpm dearme:standing-loop-audit` now renders the owner-held phone-message proof fact with a support-safe label in human-readable output while preserving exact internal fact names for audit state and guarded commands.
- **DM-GOAL-AUDIT-SAFE-OWNER-FACT** — `pnpm dearme:goal-audit -- --check` now renders the owner-held phone-message proof fact with a support-safe label in human-readable output while preserving exact internal fact names for JSON, audit state, and guarded commands.
...
- **DM-GOAL-AUDIT-PUBLIC-FIRST-RUN-MARKERS** — `pnpm dearme:goal-audit` now accepts both current personal-brand and product-incubator first-run landing markers, keeping public first-run proof green while landing copy evolves without depending on uncommitted UI files.
...
- **DM-HUMAN-HELP-VERIFIED-TODAY-GUARD** — `pnpm dearme:standing-loop-audit -- --check` now fails code-owned standing-loop checks when owner-proof or hosted-checkout blocker sections in `docs/NEEDS_HUMAN_HELP.md` are not verified for the current DearMe operating day, keeping Peter's unblock queue current while live proof remains owner-blocked.
- **DM-HUMAN-HELP-OPERATING-DATE** — Live-proof and checkout human-help handoff generators now use the DearMe America/New_York operating date shared with standing-loop freshness, preventing late-UTC blocker queue date drift.
- **DM-DEPENDENCY-REVIEW-HUMAN-HELP** — `pnpm dearme:dependency-loop-audit -- --human-help-markdown` now prints the Peter-facing dependency review queue for review-required updates, and `docs/NEEDS_HUMAN_HELP.md` records the current esbuild, TypeScript, and Vitest review blockers without autonomous bumps.
- **DM-LCP-AUDIT-INP-STABILITY** — The DearMe landing performance audit now samples the scripted interaction three times and evaluates the median INP sample, keeping the 200ms gate intact while avoiding one-off CI scheduling outliers.
- **DM-BACKLOG-CHECKBOX-DRIFT-AUDIT** — `pnpm dearme:backlog-audit -- --check` and `pnpm dearme:standing-loop-audit -- --check` now surface shipped P0/P1/P2 handoff items that remain unchecked in `CODEX-HANDOFF-TOKEN.md` while keeping the run ledger as the completion source.

...
metadata:
  openclaw:
    ...
    ticket: "DM-138"
    status: "planned"
    proxyTools: ["create_task","search_memory","get_company_documents"]
    stateMachines: []
    templates: []
    complexityRange: [3, 7]
...
Default conversational shell for the OpenClaw interface (used by Chief of Staff under the hood). Routes to specialists via `find_best_agent`.
...
Registry entry: `packages/plugins/dearme-agent-prompts/src/registry.ts` → `DEARME_ROLE_REGISTRY[role="chat"]`.

Owner ticket: `DM-138` (status: `planned`).

Plugin package that wires this skill into the DearMe outbound surface: `@paperclipai/dearme-chat`.

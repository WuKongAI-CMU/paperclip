...
function blockedHostnameMessage(hostname: string): string {
  return (
    `Hostname '${hostname}' is not allowed for this DearMe instance. ` +
    `To allow it, run the local hostname allowlist command for this install: allowed-hostname ${hostname}`
  );
}
...
export function privateHostnameGuard(opts: {
  ...
  return (req, res, next) => {
    ...
    if (!hostname) {
      const error = "Missing Host header. To allow a hostname, run the local hostname allowlist command for this install: allowed-hostname <host>.";
      if (wantsJson) {

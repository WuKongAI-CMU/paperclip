...
- [`scripts/dearme-aha-proof.ts`](../../scripts/dearme-aha-proof.ts) — first-wow proof gate over the shared first-cycle preview contract. It proves one-sentence start, 0-30s / 60-120s / 3-5min private proof order, private output package, recurring private work, phone-ready static private site export, minimum team, launch boundaries, and customer-safe language without sends, public deploys, spend, or live model calls.
- [`scripts/dearme-proof.ts`](../../scripts/dearme-proof.ts) — single local proof entrypoint that composes the first-wow, provider, and voice proof lanes without adding another product surface.
- `pnpm dearme:status` — compact coordinator/product status over the same proof map: first-wow aha proof, local no-send proof, voice semantic/review-loop proof, and live provider proof. It auto-loads `.dearme-proof.env` when that ignored local file exists.
- `pnpm dearme:release-gate` — release/readiness gate for the product question "can people use it?". Its JSON includes operator `factsNeeded` plus `operatorHandoff`, including a no-write product-receipt preview command before the local import command, customer-safe `productReadiness`, and an internal `productComparison` against Polsia, Naive/Paperclip, OpenClaw, and the DearMe architecture spine.
- `pnpm dearme:next-proof` — owner-facing live-proof handoff. It creates or preserves `.dearme-proof.env`, prints the exact approved proof facts still needed, accepts either capture flags or `--handoff-receipt-file <path>` for the product-generated handoff receipt, reports the local no-send check result from the same proof env, and prints the receipt preview/import commands before the guarded live command. With `--no-write`, receipt facts are applied only to the in-memory no-send check. With `--human-help-markdown`, it prints the Peter-facing queue entry from the same generated handoff. It does not send messages, publish, deploy, or spend.
- `pnpm dearme:payment-readiness` — no-network commercial payment-path proof. It confirms private beta can be sold through recorded receipts today, and separately checks whether hosted checkout has a customer-facing payment link plus receipt sync before any self-serve checkout claim. With `--human-help-markdown`, it prints the Peter-facing payment setup request for `docs/NEEDS_HUMAN_HELP.md` without exposing configured payment values.
- `pnpm dearme:daily-plain-summary` — builds the required Codex daily update from the run ledger plus standing-loop audit and posts it to Plain only when credentials and Peter's recipient email are configured. With `--human-help-markdown`, it prints the Peter-facing Plain configuration request without exposing configured values.
- `pnpm dearme:dependency-loop-audit` — classifies outdated dependencies into autonomous patch/safe-minor bumps and review-required major, prerelease, non-semver, and 0.x minor updates. With `--human-help-markdown`, it prints the Peter-facing dependency review queue entry without changing dependencies.
- `pnpm dearme:paid-loop-proof` — no-network paid-loop proof over the existing paid beta entitlement logic. It proves trial access blocks first-cycle work, a recorded receipt activates paid beta access, and active paid access removes the first-cycle paid-beta blocker without charging cards or calling payment APIs.
...
- [`scripts/dearme-linkedin-dm-rehearsal.ts`](../../scripts/dearme-linkedin-dm-rehearsal.ts) — no-network LinkedIn DM contract rehearsal. It reuses provider-smoke and the real `send_linkedin_dm` dispatcher through an injected partner endpoint, captures only method/header/payload/context keys, and does not replace the required guarded live proof.
- [`scripts/dearme-voice-smoke.ts`](../../scripts/dearme-voice-smoke.ts) — internal operator proof harness for deterministic Voice Gate, opt-in profile-token semantic scorer calibration, and the soft-reject review loop that blocks drift and accepts a safe rewrite.

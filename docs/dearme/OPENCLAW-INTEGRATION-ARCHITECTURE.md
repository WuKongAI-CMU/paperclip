...
- **Tool surface**: OpenClaw plugin tools register via `api.runtime.tools.register(...)`. DearMe ships outbound publishing tools (post_x, send_linkedin_dm, send_email, deploy_site, create_meta_campaign) here. Each tool calls into DearMe cloud with the user's `dm_sk_*` key and the user's per-channel OAuth token (which the user grants on first publish).

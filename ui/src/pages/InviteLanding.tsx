...
import { PRODUCT_ACCOUNT_LABEL, PRODUCT_NAME, PRODUCT_TEAM_LABEL, PRODUCT_WORKSPACE_LABEL } from "../lib/product-labels";
...
function mapInviteAuthFeedback(
  ...
  if (code === "INVALID_EMAIL_OR_PASSWORD") {
    return {
      tone: "error",
      message:
        `That email and password did not match an existing ${PRODUCT_ACCOUNT_LABEL}. Check both fields, or create an account first if you are new here.`,
    };
  }
...
export function InviteLandingPage() {
  ...
  const companyDisplayName = companyName || `this ${PRODUCT_WORKSPACE_LABEL}`;
          ...
          <section className={`${panelClassName} space-y-6`}>
                ...
                <p className="mt-2 max-w-2xl text-sm leading-6 text-zinc-300">
                  {showsAgentForm
                    ...
                    : requiresHumanAccount
                      ? `Create your ${PRODUCT_ACCOUNT_LABEL} first. If you already have one, switch to sign in and continue the invite with the same email.`
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div className="border border-zinc-800 p-3">
                ...
                <div className="mt-1 text-sm text-zinc-100">{invitedByUserName ?? PRODUCT_TEAM_LABEL}</div>
              </div>
              <div className="border border-zinc-800 p-3">
          ...
          <section className={`${panelClassName} h-fit`}>
            {showsAgentForm ? (
              <div className="space-y-4">
                ...
                <label className="block text-sm">
                  <span className="mb-1 block text-zinc-400">Run method</span>
                  <select
                    className={fieldClassName}
                    value={adapterType}
              ...
              <div className="space-y-5">
                <div>
                  ...
                  <p className="mt-1 text-sm text-zinc-400">
                    {authMode === "sign_up"
                      ? `Start with a ${PRODUCT_ACCOUNT_LABEL}. After that, you'll come right back here to accept the invite for ${companyDisplayName}.`
                      : `Use the ${PRODUCT_ACCOUNT_LABEL} that already matches this invite. If you do not have one yet, switch back to create account.`}
                  </p>
                </div>

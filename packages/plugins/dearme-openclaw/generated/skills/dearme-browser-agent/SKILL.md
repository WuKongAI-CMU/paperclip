...
Registry entry: `packages/plugins/dearme-agent-prompts/src/registry.ts` → `DEARME_ROLE_REGISTRY[role="browser-agent"]`.

Owner ticket: `DM-151` (status: `planned`).

Plugin package that wires this skill into the DearMe outbound surface: `@paperclipai/dearme-browser-agent`.

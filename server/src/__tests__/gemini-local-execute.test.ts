...
describe("gemini execute", () => {
  it("passes prompt via --prompt and injects paperclip env vars", async () => {
    ...
    try {
      ...
      expect(promptArg).toContain("DearMe runtime note:");
      ...
      expect(invocationPrompt).toContain("DearMe API access note:");
  ...
  it("uses a compact wake delta instead of the full heartbeat prompt when resuming a session", async () => {
    ...
    try {
      ...
      expect(promptArg).toContain("## DearMe Resume Delta");
      expect(promptArg).not.toContain("## Paperclip Resume Delta");

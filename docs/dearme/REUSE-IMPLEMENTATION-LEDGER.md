...
- DM-021 live-events head `71f954430914ac8682d830e27d615002afe29152`
  is absorbed by adapting its real-time value onto the current
  `LiveUpdatesProvider`: DearMe product events now refresh the existing
  Workbench, outputs, Voice & Memory, Brand OS, and paid-beta queries without a
  second websocket or polling loop.
- DM-021 source-restore head `1b0309ae16d7687371637259a1d3ac08cdcb87da`
  ...
  retired-source restore path through the shared contract, server route, API
  client, onboarding UI, and tests.
...
ledger. Treat those heads as no-replay candidates unless they advance; only the
small Voice & Memory live invalidation gap from DM-021 was still worth adapting.

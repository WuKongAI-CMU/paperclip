/**
 * Ads Manager (Meta Ads) system prompt for DearMe.
 *
 * Lineage: ported from research-captured production prompt
 * (17470 chars verbatim — densest production wisdom in the
 * corpus). 5 tools, CRITICAL rules, memory-is-hint rule,
 * budget tier table, returning-run logic, performance tier
 * thresholds, learning-phase 7-day rule, moderation recovery,
 * rate-limit recovery, partial-upload recovery, Sora 2 prompt
 * template, Meta policy rules, and activity-logging rules
 * are all preserved verbatim.
 */

export const ADS_MANAGER_PROMPT = String.raw`
...
export const ADS_MANAGER_ROLE = "ads-manager";

...
Default model tier: **deep (frontier model)**. Complexity range `5–9` (1–10).
...
Registry entry: `packages/plugins/dearme-agent-prompts/src/registry.ts` → `DEARME_ROLE_REGISTRY[role="brand-site-builder"]`.

Owner ticket: `DM-147` (status: `planned`).

Plugin package that wires this skill into the DearMe outbound surface: `@paperclipai/dearme-brand-site-builder`.

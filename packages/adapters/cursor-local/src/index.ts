...
Use when:
- You want DearMe to run Cursor Agent CLI locally as the agent runtime
...
- DearMe auto-injects local runtime skills into "~/.cursor/skills" when missing, so Cursor can discover them on local runs.
- DearMe auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.

...
Registry entry: `packages/plugins/dearme-agent-prompts/src/registry.ts` → `DEARME_ROLE_REGISTRY[role="ads-manager"]`.

Owner ticket: `DM-148` (status: `planned`).

Plugin package that wires this skill into the DearMe outbound surface: `@paperclipai/dearme-ads-manager`.

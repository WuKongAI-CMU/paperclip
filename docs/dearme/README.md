...
- `pnpm dearme:status` — compact coordinator/product status over the same proof map: first-wow aha proof, local no-send proof, voice semantic proof, and live provider proof. It auto-loads `.dearme-proof.env` when that ignored local file exists.

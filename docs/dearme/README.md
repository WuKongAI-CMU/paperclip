...
- `pnpm dearme:release-gate` — release/readiness gate for the product question "can people use it?". Its JSON includes operator `factsNeeded` plus `operatorHandoff`, customer-safe `productReadiness`, and an internal `productComparison` against Polsia, Naive/Paperclip, OpenClaw, and the DearMe architecture spine.
- `pnpm dearme:next-proof` — owner-facing live-proof handoff. It creates or preserves `.dearme-proof.env`, prints the exact approved proof facts still needed, gives a local capture command for non-secret values, then gives the no-send check and guarded live command. It does not send messages, publish, deploy, or spend.

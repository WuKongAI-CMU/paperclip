...
- The current production host proof is ready through GitHub Pages:
  `https://wukongai-cmu.github.io/dearme-private-proof/peter-studio` serves the
  exported private first-wow packet and passes `deploy_site_production` with
  `.dearme-proof.env`. Treat this as public HTTPS host proof for the current
  packet, not as proof of LinkedIn, Telegram, iMessage, or Meta delivery.
- The coordinator `.dearme-proof.env` now opts into local OpenClaw config reuse
...
- `pnpm dearme:host-provider-audit` is now the first production-host
  ...
  GitHub Pages. Vercel and Netlify remain unauthenticated, but they are no
  longer the immediate blocker for this sample packet. Do not add another local
...
- DM-170F wires the `semanticScorer` seam into the default app, Voice Gate
  ...
  profile-token semantic scorer over the existing accepted-sample voice
  profile, with sample/token minimums for calibration. It is still backstage
  and opt-in: no new route, customer setting, provider dependency, or voice
  memory store was added. Future live model/embedding work should replace or
  wrap this scorer behind the same seam.
- DM-170E adds the trained-scoring seam inside the existing Voice Gate service:
  `semanticScorer` can contribute a bounded semantic match/drift signal from a
  trained model or embedding scorer while reusing the same route, persisted
  `dm_sk_*` auth, DB-backed accepted-sample profile, and customer-safe reason
  contract. Future model work should plug into this seam and prove live scoring
  there; do not create another `/v1/voice/score` route, voice-memory table, key
  store, or review UI.
- DEA-63 / DM-170C completes the Voice Gate store boundary by adding the
  `dearme_voice_profiles` table and DB-backed `DearMeVoiceProfileStore` on the
  ...
  gap is real semantic model/embedding calibration, not another storage or
  key-issuance path. Migration `0078` also backfills earlier schema-only
  `channel_connections` and
...
- DM-174 now covers both Resend and SES inside the existing `send_email`
  ...
  through the same resolved-channel boundary rather than adding new tools.
...
| `server/src/services/dearme-deploy-site-dispatch.ts` | DM-177B/DM-177C/DM-177E | Default approved `deploy_site` dispatcher for the `dearme-cloud` channel. It keeps the existing approval/wrapper/audit path, validates lowercase safe handles, bounded artifact refs, and HTTPS custom-domain hosts, emits stable idempotent preview receipts at `dearme.app/<handle>?preview=*` or the operator-approved custom domain, and fails production/custom-domain deploys closed unless the relevant DearMe env gates are explicitly configured. |
| `server/src/services/dearme-deploy-site-dispatch-config.ts` | DM-177C/DM-177E | Env bridge for default approved `deploy_site` dispatch. It maps DearMe site host and custom-domain allow-list env into the dispatcher config so production/custom-domain receipts can be enabled by operator intent without reopening OpenClaw gateway fallback or adding a customer-facing domain settings surface. |
...
| `server/src/services/dearme-voice-gate.ts` + `server/src/services/dearme-voice-profile-store.ts` + `server/src/services/dearme-voice-semantic-scorer.ts` + `packages/db/src/schema/dearme_voice_profiles.ts` | DM-S07 / DM-170 / DEA-62 / DEA-63 / DM-170E / DM-170F | `dearMeVoiceGateService({ scorer?, profileStore?, semanticScorer? })`. Default scorer is deterministic: negative phrase rules (`ai_disclaimer`, `hype_word`, `stale_template`, `press_release_voice`, `punctuation_storm`, hidden-process language), per-artifact length floor/ceiling, concrete/evidence/shape rewards, and same-fingerprint continuity from accepted samples. The profile store is injectable, serializable, token-capped, and DB-backed on the app/handoff paths; `DEARME_VOICE_SEMANTIC_SCORER=profile-token` enables a local profile-token semantic scorer over that store without adding a route, setting, provider dependency, or standalone voice-memory service. The live fingerprint model lands by replacing or wrapping that scorer behind the same seam. |
...
|---|---|
| DM-170 | Cloud `/v1/voice/score` endpoint — Express route is shipped over the deterministic scorer and now has an injectable bounded DB-backed profile store, persisted `dm_sk_*` key auth through `agent_api_keys`, and an opt-in profile-token semantic scorer wired through the trained match/drift seam; live model calibration remains |
...
| DM-176/177/178 | LinkedIn DM partner endpoint/credential live smoke / remaining deploy_site live DNS/host smoke / Meta OAuth-Marketing API live smoke |
...
|---|---|---|
| DM-170 | DearMe cloud `/v1/voice/score` endpoint (route/store/auth/scorer seam/profile-token wiring shipped; live fingerprint calibration next) | server |
...
| DM-174 | `send_email` outbound tool via Resend/SES (Resend and SES channel dispatch shipped; live credential smoke next) | server + dearme-openclaw |
...
| DM-WOW-3M | Owner proof checklist surface | Polsia-style receipt/blocker clarity applied to the public-launch hold: the Decisions panel and status handoff now show the exact three remaining facts, the no-send check, and the owner-approval boundary without exposing backstage setup | `packages/shared/src/dearme-customer-text.ts`, `scripts/dearme-proof.ts`, `ui/src/pages/DearMeOnboarding.tsx`, `ui/src/pages/DearMeOnboarding.test.tsx` |
| DM-WOW-3N | Owner proof detail capture panel | Polsia live-proof blocker clarity plus Littlebird compact task-row grammar and Lindy action-boundary copy, reused on the same Decisions launch-proof panel so the owner knows exactly what to provide without seeing operator commands | `packages/shared/src/dearme-customer-text.ts`, `ui/src/pages/DearMeOnboarding.tsx`, `ui/src/pages/DearMeOnboarding.test.tsx` |
| DM-WOW-3N | Shared owner proof fact specs | Polsia-style setup clarity without another dashboard: one shared owner-proof fact spec now feeds Decisions copy, next-proof capture flags, and release-gate operator handoff | `packages/shared/src/dearme-customer-text.ts`, `scripts/dearme-next-proof.ts`, `scripts/dearme-release-gate.ts`, `ui/src/pages/DearMeOnboarding.tsx` |
...
| Owner live-proof detail specs | Polsia-style exact blocker clarity, centralized as customer-safe proof-fact specs and reused by the Decisions surface, release gate, and next-proof setup instead of duplicating operator-only copy | `packages/shared/src/dearme-customer-text.ts`, `packages/shared/src/dearme-customer-text.test.ts`, `scripts/dearme-next-proof.ts`, `scripts/dearme-release-gate.ts`, `ui/src/pages/DearMeOnboarding.tsx` |
| Owner proof detail capture panel | Polsia's live-proof gap becomes a three-row owner task list; Littlebird's compact todo rows inform the numbered input rows, and Lindy's action-card/safe-boundary pattern informs the per-detail boundary copy | `packages/shared/src/dearme-customer-text.ts`, `packages/shared/src/dearme-customer-text.test.ts`, `ui/src/pages/DearMeOnboarding.tsx`, `ui/src/pages/DearMeOnboarding.test.tsx` |
...
- The launch handoff checklist is the next reuse of that same loop: each
  private or delivered handoff now says what is ready, what the user should
  inspect, and why nothing public moves outside the launch boundary.
...
- The owner live-proof details now share one customer-safe fact spec across UI
  and setup commands, so the exact missing details stay readable without
  exposing operator-only terms.
- The detail capture panel turns that same spec into a three-row owner task
  list on the Decisions surface: what to provide, a safe example, and the

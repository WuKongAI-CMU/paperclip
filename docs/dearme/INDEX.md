...
- **DEA-47 approved X delivery receipts** — the current branch browser/API and UI proof shows the approved `post_x` handoff path records either a delivered receipt or the existing customer-safe connection-needed receipt; the live sample team package now renders both X delivery states on desktop/mobile with DearMe-only copy and no claim of live X publishing.
...
| 2 | days 8–14 | Voice + content loop publishing via DearMe-owned tools | DM-140, DM-142, DM-146, **DM-172** `post_x` `ChannelDispatch`, **DM-173A** X OAuth callback persistence proof writing into `channel_connections`, **DM-180** approval resolver Express route |

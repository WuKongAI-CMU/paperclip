...
| Development factory | `doc/plans/2026-05-08-dearme-symphony-operating-loop.md`; `.symphony/WORKFLOW.md`; `scripts/dearme-worktree-status.mjs`; `/Users/peter/symphony` | Symphony-style worker queue is now the cooperation spine for bounded tickets. DM-183 turns the local worktree inventory into a ticket-aware coordinator report with purpose labels and next-action buckets. The real daemon now routes by Linear team `DEA` plus `assignee: me`, because DearMe has no Linear Project. `DEA-7` is currently the active Symphony review lane for Voice-Gated Draft Handoff; coordinator integration should keep consuming that lane rather than spawning a parallel content runtime. | Workers must use Linear issue scope, this ledger, `BUILD-STATE.md`, `.symphony/WORKFLOW.md`, and `pnpm dearme:worktrees -- --summary-only --skip-dirty` before selecting old tickets. |
